// pages/w_spreadlist/w_spreadlist.js
...
Page({
   ...
   */
  data: {
    // icon3:'../../images/icon3@2x.png',
    // gengduo3:'../../images/gengduo3.png',
    ...
    "flag": 0, //查询类别 flag 0=特约商户 1=合作单位 2=传播大使 3=附近唱响(包括合作单位,特约商家)
  ...
  switchBack: function (e) {
    let that = this
    wxRequest('/homePage/getMerchants',
      { content: that.data.borderItem, flag: 0, type: that.data.type },
      function (res) {
        console.log(res)
  ...
  getShopList: function (pageStart) {
    ...
    wxRequest('/homePage/getMerchants', { area: that.data.area, flag: 2, type: that.data.type }, function (res) {
      console.log(res)
      if (res.data.status == 'success') {
        that.setData({
          result: res.data.result
        })
        let arr = that.data.result.shopList  //暂存信息到arr中
        if (that.data.result.pageStart == 1) {
          arr = [];  //清空
          that.setData({
            padding: false
          })
        ...
        if (myinfo.length <= that.data.result.pageTotal) { //如果总长度还没有一页的长度那么长
          arr = [];
          that.setData({
            padding: false,
  ...
  lowerMoreShopList: function (e) {
    if (this.data.padding) {
      this.getShopList(this.data.result.pageStart)
    }
  },
  /**
   * 生命周期函数--监听页面加载
