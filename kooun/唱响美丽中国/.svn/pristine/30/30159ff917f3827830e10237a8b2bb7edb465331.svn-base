...
Page({
  ...
  data: {
    ...
    "flag": 3, //查询类别 flag 0=特约商户 1=合作单位 2=传播大使 3=附近唱响(包括合作单位,特约商家)
  ...
  switToSDetail:function(e){
    ...
    wx.navigateTo({
      url: '/pages/wstore_detail/wstore_detail?flag=' + 3 + '&roleId=' + this.data.data[id].roleId,
    })
  },
  //获取
  switchBack:function(e){
    let that = this
    wxRequest('/homePage/getMerchants', { content: that.data.borderItem, flag:that.data.flag, type:3 }, function (res) {
      console.log(res)
  ...
  getShopList: function (pageStart) {
    ...
    { area: that.data.area, flag: that.data.flag, type: that.data.type },
     function (res) {
      console.log(res)
      if (res.data.status == 'success') {
        that.setData({
